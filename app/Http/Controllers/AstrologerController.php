...
class AstrologerController extends Controller
{
    ...
    public function view($id)
    {
        ...
        $missCallLogs =  Communication::where('astrologer_id', $astrologer->id)->with('user')->latest()->get();
        
        return view('backend.astrologers.view', compact('astrologer', 'banks', 'missCallLogs'));
    }
    ...
    public function reject(Request $request, $id)
    {
        ...
        $astrologer = Astrologer::findOrFail($id);
        $astrologer->profile_status = 'rejected';
        $astrologer->remark = $request->remark; // make sure "remark" column exists
        $astrologer->save();

        return redirect()->back()->with('success', 'Astrologer rejected with remark.');
    }
    ...
    public function callLogs(){
        ...
        $callLogs =  Communication::with('user', 'astrologer')->latest();
        
        if($from && $to):
            $callLogs->whereBetween('created_at', [$from. ' 00:00:00', $to. ' 23:59:59' ]);
        else:
            $callLogs = $callLogs->whereDate('created_at', today());
        endif;
        
        $callLogs = $callLogs->get();
        
        return view('backend.call-logs', compact('callLogs'));
    }
