        ...
        try {
            ...
            if ($request->status == 'paid') {
                ...
                if ($user->wallet != null) {
                    ...
                    $user->is_bonus = ( $request->is_bonus == true || $request->is_bonus == "true") ? 1:0;
                    
                    // logger()->info($user->is_bonus);
...
//log create
            ...
            # Admin Commisson Charge
            
            logger()->info($user);
            logger()->info($user->is_bonus);
            if ($user->is_bonus) {
                $admin_commission_percentage = 95;
                $admin_commission_amount = 0;
            
                $total_user_amount_paid = Communication::where([
                    'user_id' => $user->id,
                    'status'  => 'accept'
                ])->sum('total_amount');
            
                logger()->info($total_user_amount_paid);
                
                if ($total_user_amount_paid < 100) {
                       
                       $actual_total_amount = $total_user_amount_paid + $totalChargeAmount;  # 80 + 40 = 120
                        
                        if($actual_total_amount < 100):
                            // Commission on eligible amount
                            $admin_commission_percentage_amount = ($totalChargeAmount * $admin_commission_percentage) / 100;
                    
                            // Save commission info
                            $communication->admin_commission_percentage = $admin_commission_percentage;
                            $communication->admin_commission_amount     = $admin_commission_percentage_amount;
                    
                            // Deduct only the eligible commission
                            $totalChargeAmount = $totalChargeAmount - $admin_commission_percentage_amount ;
                        else:
                            $remaining_admin_amount_pending = 100 - $total_user_amount_paid;
                            // Commission on eligible amount
                            $admin_commission_percentage_amount = ($remaining_admin_amount_pending * $admin_commission_percentage) / 100;
